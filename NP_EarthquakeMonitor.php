...
class NP_EarthquakeMonitor extends NucleusPlugin
{
	...
	$query="create table if not exists ".sql_table('plugin_earthquakemonitor')." ( cachetime int(10) primary key default 0, data blob default '')";
	sql_query($query);
	
	/* Fill initial dummy data */
	$query="insert into ".sql_table('plugin_earthquakemonitor')." (cachetime,data) values ('1','')";
	sql_query($query);
	...
	function doSkinVar($skinType)
	{
		...
		$title = $arrayXML[$i]->title;
			...
			echo "<dd><a target='{$target}' title='{$arrayXML[$i]->description} {$arrayXML[$i]->title} ' href='{$arrayXML[$i]->link}'>{$title}</a></dd>\n";
			}
